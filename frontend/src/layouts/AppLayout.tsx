...
import {
  ...
  LogOut, Menu, X, ShoppingCart, Megaphone, Ticket, Phone, MessageSquare, Users, Calendar
} from 'lucide-react';
    ...
    <div className="h-screen flex bg-gray-50">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="flex items-center gap-1 px-4 lg:px-6 h-16 bg-white border-b border-gray-200 flex-shrink-0 z-10">
          <button className="lg:hidden p-2 text-gray-500 hover:text-gray-700 active:bg-gray-100 rounded-lg transition-colors" onClick={() => setMobileOpen(true)} aria-label="Buka menu">
        ...
        <div className={`fixed inset-0 z-50 lg:hidden transition-all duration-300 ease-in-out ${mobileOpen ? 'visible' : 'invisible'}`}>
          ...
          <div className={`absolute left-0 top-0 bottom-0 w-[260px] bg-white shadow-xl transition-transform duration-300 ease-in-out ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex items-center justify-between px-5 h-16 border-b border-gray-200">
              ...
              <button onClick={() => setMobileOpen(false)} className="p-2 text-gray-500 hover:text-gray-700 active:bg-gray-100 rounded-lg transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
